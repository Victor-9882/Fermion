PROGRAM SPLINE_2D_FERMION_BOSON
  ...
  INTEGER :: ii, i, j, NPARAM, INFO, iw, f, s !f vai numerar a equação acoplada
  INTEGER :: N_intervalG, N_intervalZ, NCOL
  INTEGER :: Nnz(100), Nng(100), Nnv(100)
  INTEGER :: Nz, Ng, Nv
  DOUBLE PRECISION :: e, mf, ms, Mtot, mu, kappa, PI, gam0, Alfa, m
  ...
  DO ii = 1, NPARAM
    ...
    CALL BUILD_ZMATRIX(zv, gv, NMZ, NMG, NMA, m, mu, kappa, PI, &
                       ...
                       ms, mf, Mtot, ZMATRIX, XMATRIX, kernel_type)
...
SUBROUTINE BUILD_ZMATRIX(zv, gv, Nmz, Nmg, NMA, m, mu, kappa, PI, &
  ...
  DOUBLE PRECISION, INTENT(IN)  :: m, mu, kappa, PI, ms, mf, Mtot
            ...
            do q = 1, Nz
              ...
              do r = 1, Nv
                ...
                ZMATRIX(index1,index2) = ZMATRIX(index1,index2) + &
                  KERNEL_UPPER(z, zq, g, gp, v, m, mu, kappa, PI, s, f, ms, mf, Mtot, kernel_type) * &
              ...
              do r = 1, Nv
                ...
                dv = DWv(r)
                ZMATRIX(index1,index2) = ZMATRIX(index1,index2) + &
                  KERNEL_LOWER(z, zq, g, gp, v, m, mu, kappa, PI, s, f, ms, mf, Mtot, kernel_type) * &
                  splg(k)*splz_q(l)*dzq*dgp*dv
              end do
...
! -----------------------------------------------------------------------
DOUBLE PRECISION FUNCTION KERNEL_UPPER(z, zp, g, gp, v, m, mu, kappa, PI, s, f, ms, mf, Mtot, kernel_type)
  IMPLICIT DOUBLE PRECISION (a-h, o-z)
  DOUBLE PRECISION, INTENT(IN) :: m, mu, kappa, ms, mf, Mtot
  INTEGER, INTENT(IN) :: s, f
  CHARACTER, INTENT(IN) :: kernel_type
  DOUBLE PRECISION :: M2_4, ku  ! k,M começam com letra i-n: precisam de declaração explícita
...
! -----------------------------------------------------------------------
! KERNEL_LOWER  —  integrando para z' em [-1, z]  (eq. 15, theta(z-z')=1)
! -----------------------------------------------------------------------
DOUBLE PRECISION FUNCTION KERNEL_LOWER(z, zp, g, gp, v, m, mu, kappa, PI, s, f, ms, mf, Mtot, kernel_type)
  IMPLICIT DOUBLE PRECISION (a-h, o-z)
  DOUBLE PRECISION, INTENT(IN) :: m, mu, kappa, ms, mf, Mtot
  INTEGER, INTENT(IN) :: s, f
  CHARACTER, INTENT(IN) :: kernel_type
  DOUBLE PRECISION :: M2_4, kd  ! k,M começam com letra i-n: precisam de declaração explícita
  ...
  KERNEL_LOWER = (1.d0-z)**2 / (32.d0*PI**2 * D0) * v**2 * Bsf / Dd**2
